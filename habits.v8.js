...
function smartHabitLabel(name){
  ...
  // common phrase-based picks (fast path)
  const phraseMap = [
    {re:/\blimit\b.*\bscreen\b/i, out:"Screen"},
    {re:/\bdrink\b.*\bwater\b/i, out:"Water"},
  ];
  for(const m of phraseMap){
    if(m.re.test(s)) return m.out;
  }
  ...
  // prefer last meaningful word, but avoid generic endings
  ...
  // capitalize first letter (keep original casing otherwise)
...
function renderAnalytics(){
  ...
  corner.innerHTML = `<div class="cap">DATE</div><div class="big">${fmtMonthDay(dates[dates.length-1])}</div><div class="small">${fmtWeekday(dates[dates.length-1])}</div>`;
