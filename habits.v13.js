if(window.__HABITS_V11_LOADED__){console.warn("habits.v11 already loaded");}else{window.__HABITS_V11_LOADED__=true;(function(){

// --- Analytics helpers (must be defined before first render) ---
function rangeDates(rangeDays, offsetDays){
  // returns [today+offset ... today+offset+rangeDays-1]
  const base = new Date();
...
function smartHabitLabel(name){
  const s = (name||"").trim();
  if(!s) return "";
  // phrase-based picks
  if(/\blimit\b.*\bscreen\b/i.test(s)) return "Screen";
  if(/\bdrink\b.*\bwater\b/i.test(s)) return "Water";

  const words = s.split(/\s+/).filter(Boolean);
  if(words.length===1){
    let w = words[0];
    if(w.length>3 && /s$/i.test(w)) w = w.replace(/s$/i,"");
    return w.charAt(0).toUpperCase()+w.slice(1);
  }

  const stop = new Set(["a","an","the","to","and","or","of","for","in","on","at","my","your","our","with"]);
  const w2 = words.filter(w=>!stop.has(w.toLowerCase()));
  const genericEnd = new Set(["time","routine","habits","habit","daily","tracker"]);
  let pick = (w2.length? w2[w2.length-1] : words[words.length-1]);
  if(genericEnd.has(pick.toLowerCase()) && w2.length>1){
    pick = w2[w2.length-2];
  }
  if(pick.length>3 && /s$/i.test(pick)) pick = pick.replace(/s$/i,"");
  return pick.charAt(0).toUpperCase() + pick.slice(1);
}

// analytics state
var analyticsView = (typeof analyticsView!=="undefined" ? analyticsView : (localStorage.getItem("habitsAnalyticsView") || "week"));
var analyticsOffsetDays = (typeof analyticsOffsetDays!=="undefined" ? analyticsOffsetDays : parseInt(localStorage.getItem("habitsAnalyticsOffsetDays") || "0", 10));
var analyticsPaintMode = (typeof analyticsPaintMode!=="undefined" ? analyticsPaintMode : (localStorage.getItem("habitsAnalyticsPaintMode") || "mark")); // mark | erase
...
function renderAnalytics(){
  ...
  corner.innerHTML = `<div class="cap">DATE</div><div class="big">${fmtMonthDay(dates[0])}</div><div class="small">${fmtWeekday(dates[0])}</div>`;
  ...
  habits.forEach(h=>{
    ...
    el.title = h.name;
    const short = smartHabitLabel(h.name);
    el.innerHTML = `<span>${escapeHtml(short)}</span>`;
    // long-press (4s) to delete habit
    let lpTimer = null;
    let lpStartX = 0, lpStartY = 0;
    const cancelLP = ()=>{ if(lpTimer){ clearTimeout(lpTimer); lpTimer=null; el.classList.remove("holding"); } };
    el.addEventListener("pointerdown",(ev)=>{
      // don't trigger on scroll wheel etc.
    ...
    el.addEventListener("pointermove",(ev)=>{
      if(!lpTimer) return;
      const dx=Math.abs(ev.clientX-lpStartX), dy=Math.abs(ev.clientY-lpStartY);
...
// expose for inline handlers (legacy)
try{
  if(typeof addHabit==="function") window.addHabit = addHabit;
  if(typeof render==="function") window.render = render;
  if(typeof renderAnalytics==="function") window.renderAnalytics = renderAnalytics;
  if(typeof markSelectedDate==="function") window.markSelectedDate = markSelectedDate;
}catch(e){}
